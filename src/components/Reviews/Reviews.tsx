...
import ReviewsCollapse from '@/components/Reviews/ReviewsCollapse'
...
export default function Reviews() {
  const { reviewsTitle, reviews = [] }: DescriptionsType =
    useProductStore.getState().descriptions
  const lenOfFeaturedReviews = 3

  const numOfMarketingTexts =
    useProductStore.getState().descriptions.marketingTexts.length
  ...
  const raw_media = useProductStore
    .getState()
    .product.media.edges.slice(
      numOfMarketingTexts + 1,
      numOfMarketingTexts + numOfReviewThumbnails + 1
    )
  ...
  const thumbnailsCounts = useProductStore
    .getState()
    .descriptions.reviews.map((review) => review.thumbnailCount)
  ...
  return (
    <Box sx={{ pb: 2 }}>
      ...
      <Grid container spacing={2}>
        {reviews
          ?.splice(0, lenOfFeaturedReviews)
          ?.map((c: ReviewType, index: number) => (
      ...
      {reviews?.length > 0 && (
        <ReviewsCollapse
          reviews={reviews}
          thumbnails={images}
          thumbnailsSliceStarts={thumbnailsSliceStarts.slice(
            lenOfFeaturedReviews
          )}
        />
      )}
    </Box>
