...
export const useProductStore = create<ProductState>((set, get) => ({
  ...
  variantIndex: 0,
  setVariantIndex: (newVariantIndex: number) =>
    set(() => ({ variantIndex: newVariantIndex })),
  variantClicked: false,
  setVariantClicked: (newVariantClicked: boolean) =>
    set(() => ({ variantClicked: newVariantClicked })),

  quantity: 1,
  setQuantity: (newQuantity: number) => set(() => ({ quantity: newQuantity })),

  descriptions: {} as DescriptionsType,
  setDescriptions: (newDescriptions: DescriptionsType) =>
    set(() => ({ descriptions: newDescriptions })),

  product: {} as ProductType,
  productID: '',
  setProductID: (new_product_id: string) =>
    set(() => ({ productID: new_product_id })),
  setProduct: async () => {
    const variables = {
